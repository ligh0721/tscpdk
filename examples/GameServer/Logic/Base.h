...
public:
    ...
    inline bool operator==(const CPoint& p) { return (p.x == x) && (p.y == y); }
    inline CPoint operator+(const CPoint& p) { return CPoint(x + p.x, y + p.y); }
    inline CPoint operator-(const CPoint& p) { return CPoint(x - p.x, y - p.y); }
    inline CPoint operator*(float f) { return CPoint(x * f, y * f); }
    inline CPoint operator/(float f) { return CPoint(x / f, y / f); }
    inline CPoint& operator+=(const CPoint& p) { x += p.x; y += p.y; return *this; }
    inline CPoint& operator-=(const CPoint& p) { x -= p.x; y -= p.y; return *this; }
