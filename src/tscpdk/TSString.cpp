...
const char* CStringHelper::PickOutWord(const char* pStr, char cLeftPair, char cRightPair, const char* pLeftSeps, const char* pRightSeps, char* pSep, size_t* pWordLen)
{
    if (!pStr)
    {
        return NULL;
...
const char* CStringLine::GetNextStringPos(size_t& uLen)
{
...
{
    size_t uLen;
    const char* pRet = GetNextStringPos(uLen);
    if (!pRet)
    {
        return -1;
    }
    uint32_t dwStrLen = uLen >= roStr.GetSize() ? roStr.GetSize() - 1 : uLen;
