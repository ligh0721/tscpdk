...
bool CHashMap<KEY, MAPPED, EXTENTION>::Init(uint32_t dwHashWidth, uint32_t dwHashHeight, const KEY& rEmptyKey)
{
    ...
    if (!AllocAndInitHashMemory(GetHeaderSize() + oTmpHdr.uHashNodeCount * GetNodeSize(), oTmpHdr))
    {
        return false;
    }
...
template <typename KEY, typename MAPPED, typename EXTENTION>
inline void* CHashMap<KEY, MAPPED, EXTENTION>::GetHeader() const
{
    return m_pHeader;
}
...
template <typename KEY, typename MAPPED, typename EXTENTION>
inline timeval* CHashMap<KEY, MAPPED, EXTENTION>::GetTimeValueFind()
{
    return &m_stTv;
}
...
template <typename KEY, typename MAPPED, typename EXTENTION>
bool CShmHash<KEY, MAPPED, EXTENTION>::Init(uint32_t dwHashWidth, uint32_t dwHashHeight, const KEY& rEmptyKey, const char* pShmName)
{
    m_sShmName = pShmName;
...
template <typename KEY, typename MAPPED, typename EXTENTION>
bool CShmHash<KEY, MAPPED, EXTENTION>::Attach(const char* pShmName)
{
    unlikely(!m_oShm.Attach(pShmName))
    {
        return false;
    }
...
{
    //LOG_DBG("DBG | Hash share memery size(%Q)", uSize);
    unlikely(!m_oShm.Init(m_sShmName.c_str(), uSize))
    {
        return false;
    }
...
template <typename KEY, typename MAPPED, typename DATA, typename EXTENTION>
inline bool CPersistentHash<KEY, MAPPED, DATA, EXTENTION>::Init(uint32_t dwHashWidth, uint32_t dwHashHeight, const KEY& rEmptyKey, const char* pShmName)
{
    unlikely ((!CShmHash<KEY, MAPPED, EXTENTION>::Init(dwHashWidth, dwHashHeight, rEmptyKey, pShmName)))
    {
        //LOG_DBG("ERR | shm init err");
        return false;
    }
    ...
    //LOG_DBG("MSG | After LoadMemory Seq: %u", ((CHashHeader*)this->GetHeader())->GetSequence());

    unlikely (!OnLoadBinlog())
    {
        return false;
    }
    //LOG_DBG("MSG | After LoadBinlog Seq: %u", ((CHashHeader*)this->GetHeader())->GetSequence());
...
template <typename KEY, typename MAPPED, typename DATA, typename EXTENTION>
inline bool CPersistentHash<KEY, MAPPED, DATA, EXTENTION>::NodeMatch(const KEY& rKey, CHashNode_& rNode)
{
    return NodeMatch(rKey, *(CHashNode*)&rNode);
}

template <typename KEY, typename MAPPED, typename DATA, typename EXTENTION>
inline bool CPersistentHash<KEY, MAPPED, DATA, EXTENTION>::NodeMatch(const KEY& rKey, CHashNode& rNode)
{
    return !memcmp(&rKey, &rNode.tKey, sizeof (KEY));
}
...
template <typename KEY, typename MAPPED, typename DATA, typename EXTENTION>
inline void CNormalPersistentHash<KEY, MAPPED, DATA, EXTENTION>::CDumpThread::OnExit(void* pExitCode)
{
    m_uHdl = 0;
}
...
template <typename KEY, typename MAPPED, typename DATA, typename EXTENTION>
bool CNormalPersistentHash<KEY, MAPPED, DATA, EXTENTION>::Init(unsigned int dwHashWidth, unsigned int dwHashHeight, const KEY& rEmptyKey, const char* pShmName, const char* pBinlogPrefix, const char* pBinlogSuffix, const char* pDumpPrefix, const char* pDumpSuffix)
{
    ...
    return CPersistentHash_::Init(dwHashWidth, dwHashHeight, rEmptyKey, pShmName);
}

template <typename KEY, typename MAPPED, typename DATA, typename EXTENTION>
bool CNormalPersistentHash<KEY, MAPPED, DATA, EXTENTION>::Attach(const char* pShmName, const char* pBinlogPrefix, const char* pBinlogSuffix, const char* pDumpPrefix, const char* pDumpSuffix)
{
    m_sBinlogPrefix = pBinlogPrefix;
    m_sBinlogSuffix = pBinlogSuffix;
    m_sDumpPrefix = pDumpPrefix;
    ...
    return CPersistentHash_::Attach(pShmName);
}
...
bool CNormalPersistentHash<KEY, MAPPED, DATA, EXTENTION>::OnWriteBinlog(const CDataCell& rData)
{
    ...
    //LOG_DBG("DBG | write binlog(%s) %d bytes", szFileName, (int)sizeof(rData));
